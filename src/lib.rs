// Copyright 2024 by contributing developers
//
...
//!
//! This library implements a rust interface to the Secret Service API
//! accessed through dbus on Linux.
//!
...
// Util contains a function to execute prompts (used in many collection and item methods, like
// delete)
//
...
impl<'a> SecretService<'a> {
    ...
    pub fn create_collection(&self, label: &str, alias: &str) -> Result<Collection, Error> {
        ...
        let collection_path: Path = {
            ...
            let created_object_path = res.get(0).ok_or(Error::NoResult)?;
            let created_path: &Path = created_object_path.inner().unwrap();

            // Check if that path is "/", if so should execute a prompt
            if &**created_path == "/" {
                let prompt_object_path = res.get(1).ok_or(Error::NoResult)?;
